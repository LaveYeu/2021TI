...
def split_licensePlate_character(plate_binary_img, k):
    """
    ...
    centroids, clusterAssment = biKmeans(dataArr, k, distMeas=distEclud)
    ...
    for i in range(len(split_list)):
        ...
        if(i == 1):
            img12 = single_character_Arr
    if(K == 1):
        return img11 # character_list中保存着每个字符的二值图数据
    elif(k == 2):
        return img11, img12# character_list中保存着每个字符的二值图数据
...
while(1):
    if (GPIO.input(17) == 0):                     # 按键如果按下，开始识别开头色卡
        ...
        print("照片拍摄完成")
        GPIO.output(10, 1)                          #照片拍摄完成后，电平拉高 发出信号，告诉 岳 小车开始出发
        ...
        dimg1 = split_licensePlate_character(dst1, 1)
        ...
        print("识别完成")
        GPIO.output(10, 0)                             #识别完成后，电平拉低 发出信号，告诉 岳 小车可以转弯

        print(pred_y1[0])
        if (pred_y1[0] == 2):
            GPIO.output(27, 1)  # 小车左拐
        else:
            GPIO.output(22, 1)  # 小车右拐
        counter = 0
        while(GPIO.input(18) == 0):                    # 读取到 岳 发的信号后， 准备开始下一轮识别
            pass
        break
pred_y1[0] = 0
...
while(1):
    if (GPIO.input(17) == 0):                         # 按键如果按下，开始识别开头色卡
        ...
        print("识别完成")
        GPIO.output(10, 0)                                #识别完成后，电平拉低 发出信号，告诉 岳 小车可以出发

        print(pred_y1[0])

        while(GPIO.input(18) == 0):                        # 到达中端 识别地点，准备识别中端数字
            pass
        ...
        if (pred_y1[0] == pred_y2[0]):
            GPIO.output(22, 1)       # 小车左拐
        else:
            GPIO.output(27, 1)        # 小车右拐
        counter = 0

        while(GPIO.input(18) == 0):                       #  准备开始下次识别
            pass
        break
pred_y1[0] = 0
...
while(1):
    if (GPIO.input(17) == 0):                  # 按键如果按下，开始识别开头色卡
        ...
        print("4识别完成")
        GPIO.output(10, 0)                         # 识别完成

        print(pred_y1[0])

        while(GPIO.input(18) == 0):                # 到识别点时给信号  远端病房1开始识别
            pass
        ...
        retval, dst = cv.threshold(gray, 55, 255, cv.THRESH_BINARY_INV)
        dst1 = remove_plate_upanddown_border(dst)
        dimg1, dimg2 = split_licensePlate_character(dst1, 2)
        print(dimg1, dimg2)
        dimg1 = resize_image(dimg1)
        dimg2 = resize_image(dimg2)

        b = torch.Tensor(dimg1)
        b = torch.unsqueeze(b, dim=0)
        b = torch.unsqueeze(b, dim=0)
        test_output = cnn(b)
        pred_y4[0] = int(torch.max(test_output, 1)[1].data.numpy().squeeze())

        b = torch.Tensor(dimg2)
        b = torch.unsqueeze(b, dim=0)
        b = torch.unsqueeze(b, dim=0)
        test_output = cnn(b)
        pred_y5[0] = int(torch.max(test_output, 1)[1].data.numpy().squeeze())

        GPIO.output(10, 0)                       #识别完成后，电平拉低 发出信号，告诉 岳 小车可以转弯

        if ((pred_y1[0] == pred_y4[0]) or (pred_y1[0] == pred_y5[0])  ):
            GPIO.output(22, 1)                   # 左转
        else:
            GPIO.output(27, 1)                   # 右转

        while(GPIO.input(18) == 0):              # 到识别点时给信号  远端病房2开始识别
            pass
        ...
        b = torch.unsqueeze(b, dim=0)
        test_output = cnn(b)
        pred_y6[0] = int(torch.max(test_output, 1)[1].data.numpy().squeeze())
        print("识别完成后")
        GPIO.output(10, 0)                       #识别完成后，电平拉低 发出信号，告诉 岳 小车可以转弯

        print(pred_y2[0])
        if (pred_y1[0] == pred_y6[0]):
            GPIO.output(22, 1)                   # 左转
        else:
            GPIO.output(27, 1)                   # 右转
        counter = 0
        while(GPIO.input(18) == 0):              # 结束
            pass
        break
time.sleep(10)
